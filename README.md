Small helper extension for Firefox. Has a few UI enhancements, working on local storage.


### Installation
Copy this repository.
Enable this extension in your browser by opening
```http
about:debugging#/runtime/this-firefox
```
Choose "Load temporary add-on" and navigate to where you copied the repository to.
Select manifest.json to load the extension.
Currently you have to reload the page after enabling the extension to receive full functionality.

There is also a Python backend prototype to use as local storage. You can find it here:
TBA

There might be other settings I set to enable this, I need to test on a fresh install.

Copyright (c) 2026 Samuli Nikkilä
Licensed under the Apache License, Version 2.0.
